...
from rl.utils.trainers import PopulationTrainer

from rl.dfo.config import DFOAgentConfig
from rl.dfo.config import DFOModelConfig

from rl.dfo.cem import CEMAgent


if __name__ == '__main__' :
    gin.parse_config_file( 'cem_config.gin' )
    ...
    if _trainerConfig.dlbackend == 'keras' :
        ...
        _model = DFOModelKeras( 'keras_model0', _modelConfig )
        _model.initialize()
    else :
    ...
    _agent = CEMAgent( 'agent0', _agentConfig, _model )

    ############################################################################

    _trainer = PopulationTrainer( _trainerConfig, _env, _agent )
    _trainer.init()
