...
LEARNING_RATE_ACTOR     = 0.0005    # learning rate used for actor network
LEARNING_RATE_CRITIC    = 0.0001    # learning rate used for the critic network
...
MAX_STEPS_IN_EPISODE    = 500       # maximum number of steps in an episode
...
class Qnetwork( nn.Module ) :
    ...
    def __init__( self, inputShape, outputShape ) :
        ...
        self.fc1 = nn.Linear( inputShape[0], 256 )
        self.fc2 = nn.Linear( 256 + outputShape[0], 128 )
    ...
    for iepisode in progressbar :
        ...
        for _ in range( MAX_STEPS_IN_EPISODE ) :
            ...
            with torch.no_grad() :
                ...
                _aa += np.array( [_noise.sample() for _ in range( 20 ) ] ).reshape( _aa.shape )
